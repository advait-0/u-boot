...
#include <asm/arch-imx8m/clock.h>
...
static int sec_mipi_dsim_config_pll(struct sec_mipi_dsim *dsim)
{
	...
	/* 🔥 FORCE PLL VALUES (DEBUG) */
	dsim->p = 11;
	dsim->m = 585;
	dsim->s = 2;
	dsim->pms = (11 << 13) | (585 << 3) | 2;
...
{
	printf("Entered sec_mipi_dsim_config_dphy\n");
	...
	uint32_t rd = dsim_read(dsim, DSIM_PHYTIMING);
	printf("PHYTIMING wrote=0x%x read=0x%x\n", phytiming, rd);
}
...
{
	printf("Enter sec_mipi_dsim_config_clkctrl\n");
	...
	int timeout = 1000;
	while (!(dsim_read(dsim, DSIM_STATUS) & STATUS_TXREADYHSCLK)) {
		udelay(10);
		if (--timeout == 0) {
			printf("HS clock not ready!\n");
			break;
		}
	}
	uint32_t status = dsim_read(dsim, DSIM_STATUS);
	printf("DSIM_STATUS after clkctrl = 0x%x\n", status);	
}
...
{
	// dsim->max_data_rate = 1000000000ULL;
	return 0;
}
